...
class TranslationWord extends Component {
  constructor(props) {
    super(props);
    this.state = {
      gameData: null,
  ...
  componentDidMount() {
    api.get("/trainings/translationWord").then((r) => {
              ...
              <Grid container direction={"row"} wrap={"nowrap"} justify={"space-between"} style={{padding: 30}}>
                <Grid item style={{flexGrow: 1}} xs={6}>
                  <Grid direction={"column"}>
                    <div className={classes.word}>{gameData[stage].translation.translation}</div>
                    {
                      selectedAnswer && (
...
export default withStyles(styles)(TranslationWord);